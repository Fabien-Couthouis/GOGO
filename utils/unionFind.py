#!/usr/bin/env python
# -*- coding: utf-8 -*-
# from: https://python-algorithms.readthedocs.io/en/stable/_modules/python_algorithms/basic/union_find.html
...
class UF:
    ...
    # fcouthouis
    def get_chains(self):
        """Get all existing chains in the union find object."""
        return [[item_id for item_id in range(len(self._id)) if self._id[item_id] == set_id]
                for set_id in list(set(self._id))]

    # fcouthouis
    def get_chain(self, set_id):
        """Get the chain with the specified set id in the union find object."""
        for chain in self.get_chains():
            if self.find(chain[0]) == set_id:
                return chain

    def __str__(self):
        """String representation of the union find object."""
        return " ".join([x for x in self._id])
